-- ============================================================================
-- ⛔ DISABLED — DO NOT RUN.  (schema.sql)
-- Superseded setup monolith. It re-creates tables and re-applies OLDER, WEAKER
-- row-level-security policies. Production is already set up with the current,
-- stricter policies (strict-access.sql). Running this could downgrade security.
-- The abort below stops the whole script if pasted into the SQL editor.
-- For an intentional from-scratch rebuild: read supabase/README.md and delete
-- ONLY this DO $$ ... $$ block first.
-- ============================================================================
DO $$ BEGIN
  RAISE EXCEPTION 'DISABLED: schema.sql is superseded and must not be run. See supabase/README.md.';
END $$;
